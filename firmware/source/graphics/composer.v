...
module composer(
    ...
    // The composed palette index is registered below.  Read the line buffers
    // using the scaled-X state that will be active after this clock edge, which
    // compensates that extra pipeline stage without shifting the pixel stream.
    wire scaled_x_advance =
        display_next_pixel && hactive && (scaled_x_counter < 'd640);
    wire [16:0] scaled_x_counter_next = display_next_line ? 17'd0 :
        scaled_x_advance ? (scaled_x_counter_r + frac_x_incr_int) :
                           scaled_x_counter_r;
    assign lb_rdidx = scaled_x_counter_next[16:7];
    ...
    // Compose the display.  Determine layer priority and sprite visibility in
    // parallel, then use one final sprite/layer mux.  This is equivalent to the
    // original five-step priority chain but has substantially less logic depth.
    wire layer0_visible = layer0_enabled && layer0_opaque;
    wire layer1_visible = layer1_enabled && layer1_opaque;
    wire [7:0] layer_data = layer1_visible ? layer1_lb_rddata :
                            layer0_visible ? layer0_lb_rddata : 8'h00;

    reg sprite_visible;
    always @* begin
        case (sprite_lb_rddata[9:8])
            2'd1: sprite_visible = sprites_enabled && sprite_opaque &&
                                       !layer0_visible && !layer1_visible;
            2'd2: sprite_visible = sprites_enabled && sprite_opaque &&
                                       !layer1_visible;
            2'd3: sprite_visible = sprites_enabled && sprite_opaque;
            default: sprite_visible = 0;
        endcase
    end

    wire [7:0] active_display_data =
        sprite_visible ? sprite_lb_rddata[7:0] : layer_data;

    always @(posedge clk) begin
        display_data <= (hactive && vactive) ? active_display_data : border_color;
    end
