...
module sprite_renderer(
    ...
    reg        sprite_on_line;
    always @* case (sprite_attr_height)
        // The maximum line indices are low-bit masks.  Testing that every bit
        // above the mask is zero is equivalent to ydiff <= maximum, without a
        // second carry-chain comparison after the y-difference subtractor.
        2'd0: sprite_on_line = (ydiff[9:3] == 0);
        2'd1: sprite_on_line = (ydiff[9:4] == 0);
        2'd2: sprite_on_line = (ydiff[9:5] == 0);
        2'd3: sprite_on_line = (ydiff[9:6] == 0);
    endcase
    wire       sprite_enabled = sprite_attr_z != 2'd0;
    // Every supported height is 2^n pixels, so its maximum line index is a
    // low-bit mask (7, 15, 31, or 63).  For an on-line sprite, subtracting the
    // line index from that mask is exactly the same as complementing those
    // bits.  XOR avoids placing a second subtractor after the y-difference.
    wire [5:0] sprite_line    = sprite_attr_vflip ? (sprite_height_pixels ^ ydiff[5:0]) : ydiff[5:0];

    // Sprite find state machine states
    parameter
        SF_CHECK_SPRITE = 3'b000,
        SF_WAIT_HIGH    = 3'b001,
        SF_WAIT_LOW     = 3'b010,
        SF_START_RENDER = 3'b011,
        SF_DONE         = 3'b100;

    // Registers used by state machine
    reg  [2:0] sf_state_r, sf_state_next;
    reg        save_hi, save_lo;
    reg        start_render_r, start_render_next;

    wire [7:0] sprite_idx_incr = sprite_idx_r + 8'd1;

    // Keep the synchronous attribute RAM address dependent on registers only.
    // A low word is requested while waiting for an accepted sprite; all other
    // search states request the current sprite's high word.
    assign sprite_idx = {
        sprite_idx_r[6:0],
        (sf_state_r == SF_WAIT_LOW) ? 1'b0 : 1'b1
    };
    ...
    always @* begin
        ...
        case (sf_state_next)
            // Check the high attribute word of the current sprite.
            SF_CHECK_SPRITE: begin
                if (sprite_idx_r[7]) begin
                    ...
                    if (sprite_enabled && sprite_on_line) begin
                        if (!render_busy) begin
                            save_hi       = 1;
                            sf_state_next = SF_WAIT_LOW;
                        end

                    end else begin
                        sprite_idx_next = sprite_idx_incr;
                        sf_state_next   = SF_WAIT_HIGH;
                    end
                end
            end

            // Allow one clock for the synchronous RAM to return the next high
            // attribute word.
            SF_WAIT_HIGH: begin
                sf_state_next = SF_CHECK_SPRITE;
            end

            // Allow one clock for the synchronous RAM to return the accepted
            // sprite's low attribute word, then prefetch the next high word.
            SF_WAIT_LOW: begin
                sprite_idx_next = sprite_idx_incr;
                sf_state_next   = SF_START_RENDER;
            end

            // Save the low word and start rendering the accepted sprite.
