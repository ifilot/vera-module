...
module video_composite(
    ...
    // These signals only change at half-line boundaries.  Keeping their state
    // explicitly avoids putting the full vertical range decoders in the pixel
    // datapath while preserving the values produced by the original decoders.
    reg v_sync         = 0;
    reg v_equalization = 1;
    reg v_active       = 0;
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            v_equalization <= 1;
            v_active       <= 0;
        end else if (h_half_line_last) begin
            case (vcnt)
                  5, 530: v_sync <= 1;
                 11, 536: v_sync <= 0;
            ...
            case (vcnt)
                  5,  17, 530, 542: v_equalization <= 0;
                 11, 524, 536, 1049: v_equalization <= 1;
                default:             v_equalization <= v_equalization;
            endcase

            case (vcnt)
                 41, 567: v_active <= 1;
                521, 1047: v_active <= 0;
                default:   v_active <= v_active;
            endcase
        end
    end
